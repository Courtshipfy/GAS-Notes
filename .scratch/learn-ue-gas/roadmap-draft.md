# UE GAS 学习路线图（初稿 DRAFT v0.1）

> **状态**：`wayfinder` effort `.scratch/learn-ue-gas` 的 ticket 07 产出初稿，合成 01–06 全部决策。**供反馈迭代**，反馈确认后再定稿/落位。
>
...
**贯穿约定**
- 技术栈（04）：**C++ 打底**（ASC 接入、AttributeSet、Ability 基类、Execution Calculation、Effect Context 扩展），**蓝图编排**（能力图、Ability Task 节点、Gameplay Cue 表现、数据/变量）。
- 深度政策（03）：**硬门槛吃透**才往下走；其余**先用后补**、用到再深挖。
